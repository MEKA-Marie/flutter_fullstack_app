...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> submit(Future<Session> Function() action) async {
    if (username.text.trim().isEmpty || password.text.isEmpty) {
      setState(() => error = 'Veuillez renseigner un identifiant et un mot de passe.');
      return;
    }
  ...
  Widget build(BuildContext context) => Scaffold(
        body: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 420), child: Padding(padding: const EdgeInsets.all(28), child: Card(child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          FilledButton.icon(onPressed: loading ? null : () => submit(() => widget.auth.login(username.text.trim(), password.text)), icon: loading ? const SizedBox.square(dimension: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.login), label: const Text('Se connecter')),
          TextButton.icon(onPressed: loading ? null : () => submit(() => widget.auth.register(username.text.trim(), password.text)), icon: const Icon(Icons.person_add_outlined), label: const Text('Créer un compte')),
          const SizedBox(height: 12),
